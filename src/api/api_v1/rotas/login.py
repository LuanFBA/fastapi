...
from src.schemas.usuario import Usuario, UsuarioCreate, LoginData, LoginSucesso
from src.crud import crud_usuario
from src.security.hash import gerar_hash, verificar_hash
from src.security.token import criar_access_token
...
def signup(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    ...
    usuario_encontrado = crud_usuario.obter_usuario_email(db=db, usuario_email=usuario.email)
...
def login(login_data: LoginData, db: Session = Depends(get_db)):
    ...
    if not usuario_db:
        raise HTTPException(status_code=401, detail=f"E-mail ou senha estão incorretos!")
