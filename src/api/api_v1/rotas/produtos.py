...
def criar_produto_usuario(produto: ProdutoCreate, usuario: Usuario = Depends(obter_usuario_logado), db: Session = Depends(get_db)):
    produto_criado = crud_produto.criar_produto_usuario(db=db, produto=produto, usuario_id=usuario.id)
    
    return produto_criado


@router.get("/", response_model=list[Produto])
def listar_produtos_usuario(usuario: Usuario = Depends(obter_usuario_logado), db: Session = Depends(get_db), skip: int = 0, limit: int = 100):
    produto_db = crud_produto.listar_produtos_usuario(db=db, usuario_id=usuario.id, skip=skip, limit=limit)
...
def atualizar_produto(produto_id: int, produto: Produto, db: Session = Depends(get_db), usuario: Usuario = Depends(obter_usuario_logado)):
    ...
    produto_atualizado = crud_produto.atualizar_produto(db=db, produto_id=produto_id, 
                                                        nome=produto.nome,
...
def remover_produto(produto_id: int, db:Session = Depends(get_db), usuario: Usuario = Depends(obter_usuario_logado)):
    produto_db = crud_produto.obter_produto(db=db, produto_id=produto_id)
